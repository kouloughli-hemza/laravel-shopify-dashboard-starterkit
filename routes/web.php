...
// Home landing page for the app Before Auth
Route::group(['prefix'=>'podify'], function() {
    Route::get('/', function () {
        return view('shopify.app');
    })->middleware(['shopify.auth'])->name('home');
    ...
    Route::middleware(['auth.shopify', 'billable'])->group(function () {
        ...
        Route::view('/app/{path?}', 'shopify.app')
            ->where('path', '.*')
            ->name('home-alt');

        Route::view('/app', 'shopify.app')->name('home');
    });
});
